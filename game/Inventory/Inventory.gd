			...
			if holding_item != null:
				if !slot.item:
					...
					if holding_item.item_name != slot.item.item_name:
						left_click_diff(event, slot)
					else:
						left_click_same(slot)
			elif slot.item:
				left_click_empty_hand(slot)
...
func left_click_diff(event: InputEvent, slot: SlotClass):
...
func left_click_same(slot: SlotClass):
	...
	if able_to_add >= holding_item.item_quantity:
		...
		holding_item.sub_item_quantity(able_to_add)
		
func left_click_empty_hand(slot: SlotClass):
	PlayerInventory.remove_item(slot)
